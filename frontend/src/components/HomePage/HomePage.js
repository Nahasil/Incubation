...
const HomePage = () => {
 ...
 async function status() {
   ...
   if (data[0]) {
     let status=data[0].pending? 'Pending':data[0].underProgress? 'Process':data[0].Approved? `Approved on Seat Number:${data[0].seat}`:data[0].Declined? 'Declined':'No Records Submit';
     setunderProcess(status)
   } 
  }
  console.log('hfhfh',underProcess);
  useEffect(() => {
    status()
  },[onSubmit])
  ...
  const onSubmit=async (datas) => {
    ...
    if (
      ...
      formData.append("upload_preset", "Project test");
      formData.append("cloud_name", "nahasil");
      //doxthu5pb
      const { data } = await axios.post(
        "https://api.cloudinary.com/v1_1/nahasil/image/upload",
        formData
      );
      if (data) {
  ...
  return (
    <div>
      ...
      {
        underProcess&&<h4 className="home" >Application Status:<span style={{ color: "green" }}>{underProcess}</span></h4>
       }
      {!(underProcess=='Pending' ||underProcess=='Process')
        &&
        <>  
